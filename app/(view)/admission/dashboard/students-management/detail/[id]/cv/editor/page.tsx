...
export default function CvEditorPage() {
  ...
  const { onUpsert: onUpsertGeneratedCvAi, onUpsertLoading } =
    useGeneratedCvAiDocuments({
      studentId: studentId as string,
      enabled: false,
    });
  ...
  const documentTitle =
    generatedDocument?.file_name ?? `${studentData?.name ?? "Student"}_CV.docx`;
  const callbackUrl = useMemo(() => {
    ...
    return `${base}/api/generate-cv-ai/onlyoffice/callback?${searchParams.toString()}`;
  }, [generatedDocument?.file_path]);

  const studentCountry =
    studentData?.stage?.country?.name ?? "Destination not set";
  const studentCampus = studentData?.name_campus ?? "Campus not set";
  const studentDegree =
    studentData?.name_degree ?? studentData?.degree ?? "Degree not set";
  const isPdfActive =
    String(generatedDocument?.file_type ?? "").toLowerCase() ===
    "application/pdf";
  ...
  const handleFinalizePdf = async () => {
    ...
    try {
      const res = await axios.post("/api/generate-cv-ai/finalize-pdf", {
      ...
      await onUpsertGeneratedCvAi(res.data?.result);
    } finally {
    ...
    >
          ...
          <Col xs={24} xl={16}>
            <Space direction="vertical" size={12} style={{ width: "100%" }}>
              <div>
                ...
                >
                  Edit CV hasil generate AI untuk{" "}
          ...
          <Col xs={24} xl={8}>
            ...
            >
              <Space direction="vertical" size={14} style={{ width: "100%" }}>
                ...
                <Typography.Text type="secondary">
                  Hasil edit akan tersimpan kembali ke file CV student yang
                  aktif.
                </Typography.Text>
                ...
                <Space direction="vertical" size={10} style={{ width: "100%" }}>
                  ...
                  {generatedDocument?.file_url ? (
                    <Button
                      type="primary"
                      icon={
                        isPdfActive ? <FilePdfOutlined /> : <FileWordOutlined />
                      }
                      size="large"
                      block
      ...
      {!fetchLoading && !generatedDocument ? (
        <Alert
          ...
          description="Generate CV terlebih dahulu sebelum membuka editor."
        />
      ) : null}
              ...
              <Space direction="vertical" size={2}>
                <Typography.Text type="secondary">
                  {isPdfActive
                    ...
                    : "Preview, edit, dan simpan CV langsung"}
                </Typography.Text>
              </Space>
