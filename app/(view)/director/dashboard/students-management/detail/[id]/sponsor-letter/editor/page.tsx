...
export default function SponsorLetterEditorPage() {
  ...
  const { onUpsert: onUpsertGeneratedSponsorLetterAi, onUpsertLoading } =
    useGeneratedSponsorLetterAiDocuments({
      studentId: studentId as string,
      enabled: false,
    });
  ...
  const documentTitle =
    ...
    `${studentData?.name ?? "Student"}_Sponsor_Letter.docx`;
  const callbackUrl = useMemo(() => {
    ...
    return `${base}/api/generate-sponsor-letter-ai/onlyoffice/callback?${params.toString()}`;
  ...
  const handleFinalizePdf = async () => {
    ...
    try {
      const res = await axios.post("/api/generate-sponsor-letter-ai/finalize-pdf", {
      ...
      await onUpsertGeneratedSponsorLetterAi(res.data?.result);
    } finally {
  ...
  const handleRevertToWord = async () => {
    ...
    await onUpsertGeneratedSponsorLetterAi({
    ...
    >
          ...
          <Col xs={24} xl={16}>
            <Space direction="vertical" size={12} style={{ width: "100%" }}>
              <div>
                ...
                >
                  Sponsor Letter Editor
                </Typography.Title>
                <Typography.Text
                  type="secondary"
                  style={{ display: "block", marginTop: 8, fontSize: 17 }}
                >
                  Edit sponsor letter hasil generate AI untuk{" "}
          ...
          <Col xs={24} xl={8}>
            ...
            >
              <Space direction="vertical" size={14} style={{ width: "100%" }}>
                ...
                <Typography.Text type="secondary">
                  Hasil edit akan tersimpan kembali ke file sponsor letter
                  student yang aktif.
                </Typography.Text>
      ...
      {!fetchLoading && !generatedDocument ? (
        <Alert
          showIcon
          type="info"
          message="Sponsor letter belum tersedia"
          description="Generate sponsor letter terlebih dahulu sebelum membuka editor."
        />
      ) : null}
              ...
              <Space direction="vertical" size={2}>
                ...
                <Typography.Text type="secondary">
                  {isPdfActive
                    ...
                    : "Preview, edit, dan simpan sponsor letter langsung dari browser."}
                </Typography.Text>
              </Space>
