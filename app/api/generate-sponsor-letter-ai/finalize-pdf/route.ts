...
function buildPdfName(sourceFileName?: string) {
  const name = String(sourceFileName ?? "sponsor-letter.docx");
...
export async function POST(req: NextRequest) {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sponsor-letter-"));
  const sourceFileName = body.source_file_name || "sponsor-letter.docx";
