...
function buildPdfName(sourceFileName?: string) {
  const name = String(sourceFileName ?? "statement-letter.docx");
...
export async function POST(req: NextRequest) {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "statement-letter-"));
  const sourceFileName = body.source_file_name || "statement-letter.docx";
